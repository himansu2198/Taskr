...
// ── Blobs ─────────────────────────────────────────────────────────────────────
const BLOBS = [
  { size: 500, color: "rgba(99,102,241,0.16)",  top: "-8%",   left: "55%",   dur: 20 },
  { size: 420, color: "rgba(59,130,246,0.13)",  top: "60%",   left: "-8%",   dur: 24 },
  { size: 340, color: "rgba(249,115,22,0.09)",  top: "10%",   left: "-5%",   dur: 17 },
  { size: 260, color: "rgba(139,92,246,0.11)",  top: "75%",   left: "70%",   dur: 28 },
];
...
export default function Register() {
  ...
  const handleSubmit = async (ev) => {
    ...
    try {
      ...
      if (res?.errors && typeof res.errors === "object") {
        ...
        if (mapped.name) { mapped.fullName = mapped.name; delete mapped.name; }
  ...
  const fields = [
    { key: "fullName", label: "Full name",       type: "text",     placeholder: "Alice Smith",      autoComplete: "name"         },
    { key: "email",    label: "Email address",    type: "email",    placeholder: "you@example.com",  autoComplete: "new-email"    },
    ...
    <motion.div style={s.page} variants={pageVariants} initial="hidden" animate="visible">
      ...
      {BLOBS.map((b, i) => (
        <motion.div
          ...
          animate={{ x: [0, -25, 20, -10, 0], y: [0, 20, -30, 15, 0], scale: [1, 1.06, 0.96, 1.03, 1] }}
      ...
      <motion.div style={s.card} variants={cardVariants} initial="hidden" animate="visible">
        ...
        <form onSubmit={handleSubmit} noValidate autoComplete="off" style={{ marginTop: 24 }}>
            ...
            {form.password.length > 0 && (
              ...
              >
                {["Weak", "Fair", "Strong"].map((level, i) => {
                  const score = form.password.length >= 10 ? 2 : form.password.length >= 6 ? 1 : 0;
                  const active = i <= score;
                  const colors = ["#f43f5e", "#f59e0b", "#10b981"];
                  return (
                    <div key={level} style={{ flex: 1 }}>
                      <motion.div
                        style={{ height: 3, borderRadius: 99, background: active ? colors[score] : "var(--surface3)" }}
                        animate={{ background: active ? colors[score] : "var(--surface3)" }}
                        transition={{ duration: 0.3 }}
                      />
                    </div>
                  );
                })}
                <span style={{ ...s.strengthLabel, color: form.password.length >= 10 ? "#10b981" : form.password.length >= 6 ? "#f59e0b" : "#f43f5e" }}>
...
const s = {
  ...
  card: {
    ...
    borderRadius: "20px", padding: "32px 34px 28px",
  ...
  heading: {
    ...
    color: "#f1f5f9", textAlign: "center", marginBottom: 5, letterSpacing: "-0.02em",
  },
  sub: { color: "var(--text-muted)", fontSize: 12, textAlign: "center" },
  label: {
    display: "block", fontSize: 12, fontWeight: 600,
    color: "var(--text-sec)", marginBottom: 6, letterSpacing: "0.01em",
  },
  ...
  strengthLabel: { fontSize: 11, fontWeight: 600, minWidth: 38 },
  btn: {
    width: "100%", padding: "12px",
    background: "linear-gradient(135deg, #f97316, #ea580c)",
    color: "#fff", border: "none", borderRadius: "10px",
    fontWeight: 700, fontSize: 14, cursor: "pointer",
    boxShadow: "0 4px 20px rgba(249,115,22,0.3)", fontFamily: "var(--font)",
  },
  ...
  dividerText: { fontSize: 11, color: "var(--text-muted)", whiteSpace: "nowrap" },
  footer: { textAlign: "center", margin: 0 },
  link: { color: "var(--orange)", fontSize: 13, fontWeight: 600, textDecoration: "none" },
};